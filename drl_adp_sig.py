...
class ActorCritic(nn.Module):
    def __init__(self) -> None:
        ...
        self.action = nn.Sequential(
            layer_init(nn.Linear(256, sum(action_space)))
        )
...
class Agent:
    ...
    def get_sample_actions(self,states, unit_masks):
        ...
        self_action_bias_mask = np.zeros((self.num_envs, sum(self.action_space)),dtype=np.float32)
        for i in range(self.num_envs):
            game:Game = self.env.games[i]
            action = self.self_ai.get_action(game)
            self_action_bias_mask[i] = action.action_to_one_hot(width, height, w=adp_weight)
            if np.sum(self_action_bias_mask[i])==0:
                self_action_bias_mask[i] = np.ones(sum(self.action_space),dtype=np.float32)
    
        action_distris = self.net.get_action_distris(states)
        action_adp_distris = action_distris + torch.Tensor(self_action_bias_mask)
